 ...
 */
final class GraphicFactoryImpl
        implements GraphicFactory
{
    ...
    /** Graphics device. */
    private static final GraphicsDevice DEV = GraphicFactoryImpl.ENV.getDefaultScreenDevice();
    /** Graphics configuration. */
    private static final GraphicsConfiguration CONFIG = GraphicFactoryImpl.DEV.getDefaultConfiguration();
    ...
    static Cursor createHiddenCursor()
    {
        ...
        final BufferedImage buffer = GraphicFactoryImpl.getBuffer(UtilityImage.applyMask(cursor, ColorRgba.BLACK));
    ...
    {
        final BufferedImage buffer = GraphicFactoryImpl.CONFIG.createCompatibleImage(width, height,
                GraphicFactoryImpl.getTransparency(transparency));
    ...
    public ImageBuffer getImageBuffer(InputStream inputStream, boolean alpha) throws IOException
    {
        ...
        final BufferedImage image = GraphicFactoryImpl.CONFIG.createCompatibleImage(buffer.getWidth(),
    ...
    {
        final BufferedImage buffer = GraphicFactoryImpl.CONFIG.createCompatibleImage(imageBuffer.getWidth(),
                imageBuffer.getHeight(), GraphicFactoryImpl.getTransparency(imageBuffer.getTransparency()));
        final Graphics2D g = buffer.createGraphics();

        g.setComposite(AlphaComposite.Src);
        g.drawImage(GraphicFactoryImpl.getBuffer(imageBuffer), 0, 0, null);
        g.dispose();
    ...
    {
        final BufferedImage alpha = GraphicFactoryImpl.CONFIG.createCompatibleImage(imageBuffer.getWidth(),
    ...
    {
        final BufferedImage image = GraphicFactoryImpl.getBuffer(imageBuffer);
        ...
        for (int y = 0; y < col; y++)
        {
            for (int x = 0; x < row; x++)
            {
                images[frame] = GraphicFactoryImpl.CONFIG.createCompatibleImage(width, height, transparency);
                final Graphics2D g = images[frame].createGraphics();
                GraphicFactoryImpl.optimizeGraphics(g);
    ...
    public ImageBuffer rotate(ImageBuffer imageBuffer, int angle)
    {
        ...
        final int transparency = image.getColorModel().getTransparency();
        final BufferedImage rotated = GraphicFactoryImpl.CONFIG.createCompatibleImage(w, h, transparency);
    ...
    public ImageBuffer resize(ImageBuffer imageBuffer, int width, int height)
    {
        final BufferedImage image = GraphicFactoryImpl.getBuffer(imageBuffer);
        final int transparency = image.getColorModel().getTransparency();
        final BufferedImage resized = GraphicFactoryImpl.CONFIG.createCompatibleImage(width, height, transparency);
    ...
    public ImageBuffer flipHorizontal(ImageBuffer imageBuffer)
    {
        final BufferedImage image = GraphicFactoryImpl.getBuffer(imageBuffer);
        final int w = image.getWidth(), h = image.getHeight();
        final BufferedImage flipped = GraphicFactoryImpl.CONFIG.createCompatibleImage(w, h, image.getColorModel()
                .getTransparency());
    ...
    public ImageBuffer applyFilter(ImageBuffer imageBuffer, Filter filter)
    {
        ...
        {
            case BILINEAR:
                kernel = new Kernel(3, 3, GraphicFactoryImpl.BILINEAR_FILTER);
                break;
            default:
            ...
            int refSize)
    {
        ...
        final BufferedImage raster = GraphicFactoryImpl.CONFIG.createCompatibleImage(image.getWidth(),
    ...
    {
        GraphicFactoryImpl.optimizeGraphicsSpeed(g);
    }
}
