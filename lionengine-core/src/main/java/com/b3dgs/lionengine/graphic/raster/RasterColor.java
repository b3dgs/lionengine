...
public final class RasterColor
{
    ...
    public static RasterColor load(RasterData data, int m, int i, boolean smooth)
    {
        ...
        if (smooth)
        {
            if (m == 0)
            {
                start = UtilRaster.getRasterColor(i, data, MAX_RASTERS);
                end = UtilRaster.getRasterColor(i + 1, data, MAX_RASTERS);
            }
            else
            {
                start = UtilRaster.getRasterColor(MAX_RASTERS - i, data, MAX_RASTERS);
                end = UtilRaster.getRasterColor(MAX_RASTERS - i - 1, data, MAX_RASTERS);
            }
        }
        else
        {
