...
public class XmlFactoryTest
{
    ...
    private void testWrongWriteXml() throws IOException
    {
        try
        {
            Stream.saveXml(Stream.createXmlNode("child"), Medias.create(Constant.EMPTY_STRING));
            Assert.fail();
        }
        ...
        if (file.mkdir())
        {
            try
            {
                Stream.saveXml(Stream.createXmlNode("child"), Medias.create(file.getPath()));
                Assert.fail();
            }
            catch (final LionEngineException exception)
            {
    ...
    private void testWrongReadXml()
    {
        ...
        try
        {
            Stream.loadXml(Medias.create("malformed.xml"));
            Assert.fail();
        }
        catch (final LionEngineException exception)
        {
